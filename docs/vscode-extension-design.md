...
**구현:** 실제 구현은 `editor/vscode/` 단일 패키지로 되어 있다. (본문의 `packages/react-native-mcp-ui`, `packages/react-native-mcp-vscode` 구조는 설계 단계 참고용이다.)
...
## 패키지 구조 (현재 구현: editor/vscode 단일 패키지)
...
│   │   ├── ws-client.ts               ← WebSocket 클라이언트 (12300, extension-init/getDevices/eval)
│   │   ├── status-bar.ts              ← 연결 상태 표시
│   │   ├── webview/
│   │   │   ├── panel-manager.ts       ← WebviewPanel
│   │   │   ├── sidebar-view-provider.ts
│   │   │   └── message-handler.ts     ← webview ↔ extension 메시지
│   │   ├── tree-view/
│   │   │   └── component-tree-provider.ts
│   │   └── codelens/
│   │       └── accessibility-diagnostics.ts
│   └── webview/                       ← Webview (React, IIFE 번들)
│       ├── App.tsx
│       ├── panels/
│       │   ├── ConsolePanel.tsx
│       │   ├── NetworkPanel.tsx
...
│       └── styles/
├── tsdown.config.ts                   ← extension(CJS) + webview(IIFE) 이중 빌드
└── package.json
```

### 레이어 분리 (현재 구현 기준)

| 레이어     | 위치 (editor/vscode 내)    | 역할                                       |
| ---------- | -------------------------- | ------------------------------------------ |
| **데이터** | `mcp-server` (별도 패키지) | MCP 서버, WebSocket 12300                  |
| **Shell**  | `src/extension/`           | VS Code API, ws-client, TreeView, CodeLens |
| **UI**     | `src/webview/`             | React 패널 (Console/Network/State/Renders) |

설계 단계에서 논의했던 `packages/react-native-mcp-ui` + `packages/react-native-mcp-vscode` 분리는 적용하지 않고, 단일 패키지 안에 extension(Shell)과 webview(UI)를 함께 두고 있다.
