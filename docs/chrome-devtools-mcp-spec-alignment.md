...
| `take_snapshot`         | 🔲 예정 | a11y 대신 컴포넌트 트리(Fiber/testID) 기반 uid 스냅샷.                                                                              |
| `evaluate_script`       | ✅      | Chrome과 동일: `function`(string), `args`(array). WebSocket eval로 앱에서 실행.                                                     |
| `list_console_messages` | ✅      | CDP 가로채기(/**mcp_cdp_events**)에서 Runtime.consoleAPICalled 등 수집. `pageIdx`, `pageSize`, `types`, `includePreservedMessages`. |
...
| 도구                      | 지원     | 비고                                                           |
| ------------------------- | -------- | -------------------------------------------------------------- |
| `list_pages`              | ✅       | RN은 단일 앱. 페이지 1개(id=1, title="React Native App") 반환. |
| `select_page`             | ✅ no-op | 단일 페이지만 있으므로 선택 유지.                              |
...
| 도구                             | 지원 | 비고                                                                                       |
| -------------------------------- | ---- | ------------------------------------------------------------------------------------------ |
| `click`                          | ✅   | testID(uid) 기반. `evaluate_script`로 triggerPress 호출. 기존 click_component와 동일 개념. |
| `fill`                           | 🔲   | 예정.                                                                                      |
| 기타 (drag, hover, press_key 등) | 🔲   | 예정.                                                                                      |
...
- **pageId**: RN은 단일 앱이므로 `list_pages`는 항상 1개, `pageId`는 1(또는 0)로 고정.
