...
| `assertValue`       | `selector`, `value`                  | Input 값 확인                                   |
...
| `dashboard`      | runs.json + index.html    | 최근 런·flaky·스텝 상세, 웹 대시보드       |

**리포터 확인 방법**: HTML은 실행 후 `output/report.html`을 브라우저로 열어 확인한다. Slack은 채널 도착 여부, GitHub PR은 PR 코멘트 또는 `output/pr-comment.md`로 확인한다. Dashboard는 `-r dashboard -o <dir>` 실행 후 `test report show -o <dir>`(또는 `bun run dashboard:show`)로 웹 대시보드를 연다. 자세한 사용법과 옵션은 [e2e-yaml-reference.md](e2e-yaml-reference.md)의 CLI·리포터 섹션 및 document 사이트의 E2E 대시보드 문서를 참고한다.
...
Phase별 상세 상태는 아래 Phase 테이블을 기준으로 한다.
...
Phase E+F 완료:
  ...
  └── 리포터: console, junit, json, html, slack, github-pr, dashboard ✅
```
...
| **F** | CI 리포트               | E         | **✅ 완료** | console / junit / json / html / slack / github-pr / dashboard 리포터              |
...
Phase 0~F 완료 상태를 기준으로, 당시 결정과 아직 열린 선택지는 아래와 같다.

| 항목                 | 선택지                                         | 상태                    |
| -------------------- | ---------------------------------------------- | ----------------------- |
| SDK 패키지 위치      | 모노레포 내 새 패키지 vs 서버 패키지 내 export | 결정됨 (서버 패키지 내) |
| YAML 스키마 표준     | 자체 정의 vs Maestro 호환                      | 결정됨 (자체 정의)      |
| bun test 통합        | bun test matcher 제공 여부                     | 보류                    |
| ~~병렬 테스트 실행~~ | ~~다중 디바이스 활용 병렬 실행~~               | **계획에서 제외**       |
