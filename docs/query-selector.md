...
- `uid`를 `evaluate_script`의 `measureView(uid)`에 전달하면 좌표 획득 가능

### query_selector_all (복수)

```json
[
  { "uid": "item-0", "type": "Pressable", "text": "항목 1", "hasOnPress": true },
  { "uid": "item-1", "type": "Pressable", "text": "항목 2", "hasOnPress": true }
]
```
...
```
1. query_selector('Pressable:text("로그인")') → uid 획득
2. evaluate_script(measureView(uid)) → { x, y, width, height } 획득
3. tap(platform, x + width/2, y + height/2) → 중앙 탭
```
...
### 클릭 가능 요소 목록 (list_clickables 대체)

```
query_selector_all(':has-press')
```

### 모든 텍스트 노드 (list_text_nodes 대체)
