...
**목표**: 비동기 UI 변화를 안정적으로 기다리는 유틸리티. 구현: `AppClient.waitForText`/`waitForVisible`/`waitForNotVisible`, assert 도구 `timeoutMs`/`intervalMs` 폴링.
...
**목표**: 테스트 표현력을 높이는 assertion 확장. 구현: MCP 도구 `assert_not_visible`, `assert_element_count`, AppClient `assertCount`/`assertValue` 등.
...
**목표**: 코드 작성 없이 YAML로 E2E 테스트 정의 및 실행. 구현: `packages/react-native-mcp-server/src/test/` (parser, runner, reporters), CLI: `node dist/test/cli.js run <yaml>`.
