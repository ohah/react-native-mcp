# 문서 메뉴

React Native MCP 프로젝트 문서 목차.

---

## E2E / YAML

- [E2E 테스트 계획](e2e-test-plan.md)
- YAML 레퍼런스 → [published docs](../document/docs/ko/testing/e2e-yaml-reference.md)로 이동

---

## 가이드

- [**Expo 검증 가이드**](expo-guide.md) — Expo 프로젝트 설정, Dev Client / Expo Go 호환성, 트러블슈팅

---

## 개요

- [개요 및 주요 기능](overview.md)
- [기술 스택](tech-stack.md)
- [아키텍처](architecture.md)

---

## 개발

- [개발 명령어](development-commands.md)
- [코딩 스타일 가이드라인](coding-style.md)
- [행동 가이드라인 (AI/에이전트)](behavioral-guidelines.md)

---

## 규칙

- [커밋 규칙](commit-rules.md)
- [PR 규칙](pr-rules.md)

---

## 설계·참고

- [설계 (DESIGN)](DESIGN.md)
- [쿼리 셀렉터](query-selector.md)
- [CDP 이벤트 수집 설계](cdp-event-collection-design.md)
- [CDP 전용 전환 시 변경 사항](cdp-only-migration.md)
- [앱 연결 안 됨 원인 분석](troubleshooting-app-connection.md)
- [앱 연결 connected false](troubleshooting-app-connected-false.md)
- [idb 설치 가이드](idb-setup.md)
- [Chrome DevTools MCP 스펙 정렬](chrome-devtools-mcp-spec-alignment.md)
- [네트워크 모니터링 (CDP)](network-monitoring-via-cdp-analysis.md)
- [IDB 레퍼런스](IDB_REFERENCE.md)
- [ADB 레퍼런스](ADB_REFERENCE.md)
- [XCUITest/WDA 레퍼런스](XCUITEST_WDA_REFERENCE.md)
