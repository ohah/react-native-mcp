...
서버는 stderr에 짧은 로그만 출력한다.

- `[react-native-mcp-server] WebSocket server listening on ws://localhost:12300`
- `[react-native-mcp-server] Health server listening on http://127.0.0.1:12301 ...`
- `[react-native-mcp-server] WebSocket client connected` / `disconnected`
- `[react-native-mcp-server] Running on stdio`

구조화 로그(JSON 등)나 로그 레벨 설정은 현재 제공하지 않는다.
