...
## `appConnected: false, devices: []` 권장 확인 순서

`get_debugger_status`에서 `appConnected: false`, `devices: []`가 나오면 아래 순서대로 확인한다.
...
3. **실행 환경 확인**
   - iOS 시뮬인지, Android 에뮬/실기기인지 확인. (에뮬/실기기는 현재 지원 안 됨.)
4. **앱 로그 확인**
   - `[MCP] runtime loaded` → `[MCP] Connected to server` 순서로 나오는지 확인.

> 사용자 대상 트러블슈팅은 [rspress 문서](../../document/docs/ko/mcp/troubleshooting.md) 참고.
