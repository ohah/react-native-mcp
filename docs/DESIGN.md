...
- [ ] Metro Plugin 확장
  - [ ] 컴포넌트 메타데이터 수집
  - [ ] 소스맵 연동
  - **소스 위치 추론 (\_debugStack + 소스맵)**: React 19에서 `_debugSource` 제거됨. 대안: Fiber `_debugStack.stack`에서 `(bundleUrl, line, column)` 파싱 → Metro에 `inlineSourceMap=true`로 번들 요청 → 소스맵으로 `originalPositionFor` → 원본 `(source, line, column)` 반환. 스크립트: `scripts/symbolicate-stack.mjs`. 앱이 `inlineSourceMap=false`로 로드한 URL도 서버에서 **ensureInlineSourceMap**으로 `inlineSourceMap=true`로 덮어써 요청함(symbolicate.ts).
  - **get_component_source 도구**: selector 또는 uid로 **한 컴포넌트**를 지정하면, 해당 Fiber의 `_debugStack.stack`(React가 해당 컴포넌트 렌더 시 기록한 호출 스택)을 파싱해 ref 목록을 얻고, 서버가 각 ref를 심볼리케이트해 원본 (filePath, lineNumber, columnNumber) 반환. **앱 소스 우선**: node_modules/react, node_modules/react-native, runtime.js는 제외하고, 그 외 소스(앱/데모 코드)가 있으면 그 위치를 반환; 없으면 첫 번째로 해석된 프레임을 fallback으로 반환. **토큰 절약**: "StepLayout 수정해줘"처럼 컴포넌트 기준 요청 시, 코드베이스 검색 없이 한 번의 도구 호출로 해당 파일·라인을 얻어 수정할 수 있음.
...
사용자 관점 요약은 [known-limitations-and-risks.md](known-limitations-and-risks.md)를 참고한다.
