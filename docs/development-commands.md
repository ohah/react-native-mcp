...
- **Android**: 워크플로가 연결 기기(`adb devices -l`) 및 AVD 목록(`emulator -list-avds`)을 로그에 출력한다. 사용 AVD 우선순위: `examples/demo-app/e2e/.android-emulator-device` 파일 첫 줄 > repo/org 변수 `ANDROID_EMULATOR_DEVICE` > 기본 `avd-34`. 에뮬레이터 해상도는 `-skin 1920x1200`, 방향은 세로(portrait)로 `adb shell settings put system user_rotation 0` 등으로 고정(CI 일관성용).
