...
확장은 사이드바에 두 패널을 아코디언처럼 펼치고 접을 수 있게 표시합니다. **DevTools**(Console, Network, State, Renders)와 **Component Tree**. 아래는 데이터만 목업한 더미입니다.
...
[**React Native MCP DevTools**](https://marketplace.visualstudio.com/items?itemName=ohah.react-native-mcp-devtools)에서 설치하거나, VS Code에서 `Ctrl+Shift+X` → `React Native MCP DevTools` 검색 → 설치.

> 로컬 빌드가 필요하면 아래 .vsix 설치를 사용하세요.
