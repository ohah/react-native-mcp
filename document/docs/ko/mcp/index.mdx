...
| 옵션 / 환경 변수           | 설명                                                                                         |
| -------------------------- | -------------------------------------------------------------------------------------------- |
| `renderHighlight`          | 위 표 참고. 앱 로드 시 리렌더 시각 오버레이.                                                 |
| `REACT_NATIVE_MCP_ENABLED` | Metro 실행 시 `true`로 설정하면 프로덕션 빌드에서도 MCP 활성화. 보안상 기본값은 꺼짐.        |
| testID 주입                | 프리셋이 셀렉터용 `testID`(및 선택적 `displayName`)를 주요 요소에 자동 주입. 별도 설정 없음. |
