...
| Option                                     | Description                                                                                                   |
| ------------------------------------------ | ------------------------------------------------------------------------------------------------------------- |
| `renderHighlight: true`                    | Enable on load, style `'react-mcp'` (cyan, default).                                                          |
| `renderHighlight: { style: 'react-mcp' }`  | Same as above.                                                                                                |
| `renderHighlight: { style: 'react-scan' }` | Enable on load, style `'react-scan'` (purple).                                                                |
| Omitted or `false`                         | Off by default. You can still start it via MCP tools or [VS Code DevTools](./vscode-extension) Renders panel. |
...
### Babel preset options (reference)

| Option / env               | Description                                                                                                       |
| -------------------------- | ----------------------------------------------------------------------------------------------------------------- |
| `renderHighlight`          | See table above. Visual re-render overlay at app load.                                                            |
| `REACT_NATIVE_MCP_ENABLED` | When running Metro, set to `true` to enable MCP in production builds. Default off for security.                   |
| testID injection           | The preset injects `testID` (and optional `displayName`) on key elements for selectors. No extra config required. |
