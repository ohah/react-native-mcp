import ExtensionSidebarPreview from '../../components/ExtensionSidebarPreview';
...
The extension shows two panels in the sidebar, each expandable/collapsible like an accordion: **DevTools** (Console, Network, State, Renders) and **Component Tree**. Below is a dummy preview with mock data only.

<ExtensionSidebarPreview />
...
Install from [**React Native MCP DevTools**](https://marketplace.visualstudio.com/items?itemName=ohah.react-native-mcp-devtools), or in VS Code: `Ctrl+Shift+X` → Search `React Native MCP DevTools` → Install.

> For local builds, use the .vsix installation below.
