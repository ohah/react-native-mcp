...
Once connected, **50+ tools** are available across multiple categories — interaction, assertions, screen capture, network mocking, state inspection, render profiling, video recording, visual comparison, accessibility, and more.

Ask the AI to "take a snapshot of the app" or "tap the login button" and it will call these tools automatically.

For detailed parameters, examples, and platform-specific tips for every tool, see the **[Tool Reference](./tools/overview)**. For real-world usage scenarios, check out the **[Cookbook](./cookbook/overview)**.
