          ...
          if (webViewTestIdValue != null) {
            ...
            const userRefExpr =
              refAttr?.value && t.isJSXExpressionContainer(refAttr.value)
                ...
                : null;
            if (userRefExpr != null && !t.isJSXEmptyExpression(userRefExpr))
              bodyStatements.push(
                t.ifStatement(
                  t.binaryExpression('!=', t.cloneNode(userRefExpr), t.nullLiteral()),
                  t.blockStatement([
                    t.ifStatement(
                      t.binaryExpression(
                        '===',
                        t.unaryExpression('typeof', t.cloneNode(userRefExpr)),
                        t.stringLiteral('function')
                      ),
                      t.expressionStatement(
                        t.callExpression(t.cloneNode(userRefExpr), [t.identifier('r')])
                      ),
                      t.expressionStatement(
                        t.assignmentExpression(
                          '=',
                          t.memberExpression(t.cloneNode(userRefExpr), t.identifier('current')),
                          t.identifier('r')
                        )
                      )
                    ),
