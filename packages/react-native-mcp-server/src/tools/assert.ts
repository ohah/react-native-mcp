...
export function registerAssert(server: McpServer, appSession: AppSession): void {
  ...
  registerTool(
    server,
    'assert_text',
    'Assert text exists on screen. Supports polling via timeoutMs.',
    assertTextSchema,
    async (args: unknown) => {
  ...
  registerTool(
    server,
    'assert_visible',
    'Assert element matching selector is visible. Supports polling via timeoutMs.',
    assertVisibleSchema,
    async (args: unknown) => {
  ...
  registerTool(
    server,
    'assert_not_visible',
    'Assert element matching selector is NOT visible. Supports polling via timeoutMs.',
  ...
  registerTool(
    server,
    'assert_element_count',
    'Assert element count matching selector. Use expectedCount or minCount/maxCount range.',
