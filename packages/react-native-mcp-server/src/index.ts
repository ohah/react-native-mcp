  ...
  const server = new McpServer(
    ...
    },
    {
      instructions: [
        ...
        '',
        '### Token-efficient tool selection (important)',
        'Screenshots consume significantly more tokens than text-based tools. Always prefer text tools first:',
        '| Priority | Tool | Use case |',
        '|----------|------|----------|',
        '| 1st | `query_selector`, `assert_text`, `assert_visible` | Verify native RN elements (low cost) |',
        '| 2nd | `webview_evaluate_script` | Verify/interact with WebView DOM (low cost) |',
        '| 3rd | `describe_ui` | Explore UI tree when selectors are unknown (medium cost) |',
        '| 4th | `take_screenshot` | Only when visual layout verification is needed (high cost) |',
        '',
        '- After one screenshot for orientation, switch to text-based tools for subsequent checks.',
        '- To verify a checkbox state: use `webview_evaluate_script` with `document.querySelector("input").checked`, NOT a screenshot.',
        '- To confirm navigation succeeded: use `assert_text` with expected screen text, NOT a screenshot.',
        '',
        '### Screenshots',
